 ...
 * @author Rene Argento
 */
public class QueueWithFiveStacks<Item> implements Iterable<Item> {
    ...
    public Item dequeue() {
        ...
        Item item;
        if (!isPerformingRecopy) item = head.pop();
        else {
    ...
    private class QueueWithStacksIterator implements Iterator<Item> {
        ...
        private final Iterator<Item> headIterator;
        private final Iterator<Item> reverseTailCopyIterator;

        public QueueWithStacksIterator() {
            index = 0;
            Stack<Item> reverseTailCopy = new Stack<>();
